...
class Emitter():
    ...
    *    @param typ the type of the constant
    ...
    def emitALOAD(self, in_, frame):
        ...
        elif type(in_) is ArrayType or type(in_) is cgen.ClassType or type(in_) is Id or type(in_) is StringType:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        elif type(inType) is ArrayType or type(inType) is cgen.ClassType or type(inType) is Id or type(inType) is StringType:
    ...
    * @param in the type descriptor of the method.
    ...
    def emitINVOKEINTERFACE(self, lexeme, in_, num, frame):
        #lexeme: String
        #in_: Type
        #frame: Frame
    ...
    *   generate ior
    ...
    def emitREOP(self, op, in_, frame):
        ...
        if type(in_) is IntType or type(in_) is BoolType:
            ...
            result.append(self.jvm.emitINVOKEVIRTUAL("java/lang/String/compareTo", self.getJVMType(MType([StringType()], IntType()))))
            if op == ">":
                result.append(self.jvm.emitIFLE(labelF))
            elif op == ">=":
                result.append(self.jvm.emitIFLT(labelF))
    ...
    *   @param in the type of the local array variable.
    ...
    def emitNEWARRAY(self, in_, frame):
        ...
        elif type(in_) is cgen.ClassType or type(in_) is Id or type(in_) is StringType:
            return self.jvm.emitANEWARRAY(self.getFullType(in_))
        elif type(in_) is IntType or type(in_) is FloatType or type(in_) is BoolType:
    ...
    *   .super java/lang/Object<p>
    '''
    def emitPROLOG(self, name, parent, interface):
        #name: String
        #parent: String
        ...
        if interface:
            [result.append(self.jvm.emitIMPLEMENTS(i)) for i in interface]
        return ''.join(result)
    
    def emitIPROLOG(self, name, parent):
        #name: String
        #parent: String

        result = list()
        result.append(self.jvm.emitSOURCE(name + ".java"))
        result.append(self.jvm.emitINTERFACE("abstract " + name))
        result.append(self.jvm.emitSUPER("java/lang/Object" if parent == "" else parent))
